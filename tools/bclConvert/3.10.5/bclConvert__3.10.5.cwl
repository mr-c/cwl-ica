...
doc: |
    Runs the BCL Convert application through dragen
...
hints:
  ResourceRequirement:
    ...
    ramMin: 64000
  DockerRequirement:
    dockerPull: ""  # FIXME - dragen 3.10 docker container

requirements:
  InlineJavascriptRequirement:
    expressionLib:
      - $import: typescript-expressions/bclconvert__3.10.5.cwljs
  SchemaDefRequirement:
    types:
      - $import: ../../../schemas/samplesheet/1.0.0/samplesheet__1.0.0.yaml
    ...
    listing:
      - entryname: "run_bclconvert.sh"
        entry: |
          ...
          # Extract the ora reference
          if [[ "$(is_not_null(inputs.ora_reference))" == "true" ]]; then
            tar -xzf "$(inputs.ora_reference.path)" "$(get_ora_reference_path())"
          fi
...
arguments:
  - position: -1
    valueFrom: "run_bclconvert.sh"
  - position: 1
    prefix: "--bcl-conversion-only"
    valueFrom: "true"
inputs:
  # Required inputs
  bcl_input_directory:
  ...
  samplesheet:
    ...
    type: ../../../schemas/samplesheet/1.0.0/samplesheet__1.0.0.yaml#samplesheet
    inputBinding:
      prefix: --sample-sheet
  ...
  ora_reference:
    label: ora reference
    doc: |
      The ora reference tar ball
    type: File?
    inputBinding:
      prefix: "--ora-reference"
      valueFrom: "$(get_ora_reference_path())"
  bcl_only_lane:
...
outputs:
  ...
  fastq_list_rows:
    ...
    outputBinding:
      ...
      outputEval: |
        ${
          return get_fastq_list_rows_from_fastq_list_csv(self[0]);
        }
